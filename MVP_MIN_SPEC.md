...
- 后端开发 2：罗建明
- 前端开发 1（UI 向）：钱满亮
  - 可参考关注：深色主题、卡片与列表、动效与提示、空态/错误态。
- 前端开发 2（数据+资产管理）：金裕涵
  - 可参考关注：Preset 配置（或 JSON 驱动）、历史列表、筛选与分页、持久化。
- 前端开发 3（后端对接项）：项科深
  - 可参考关注：API service、鉴权拦截器、Polling、错误重试、统一通知。
- 前期调研 + 后期对比测试：魏云翔
  - 可参考关注：相关调研、性能与体验对比、Demo 可靠性测试。
...
- 构建：`dotnet build`（csproj）；`.sln` 仅 VS 备选；前端 `pnpm dev`。`dotnet watch`可以全部替代。
